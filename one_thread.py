...
class SignDetector(object):
    ...
    def loadNetSign(self, weightPath="/sign.weights", configPath="/yolov3-tiny-obj.cfg"):
...
class TFModels(SignDetector):
    def __init__(self):
        ...
        self.lane_model = load_model(path+'/models/model.h5')
        self.sign_model = load_model(path+'/models/signgray.h5')
        sys.path.insert(1, path+'/models')

        self.signDetector_net = self.loadNetSign()

    def caculateAngle(self, center, width, line):
        temp = math.atan(float(abs(float(center)-float(width/2))/float(line)))
    ...
    def predictCenter(self, image, line):
        global lane_model
        ...
        angle = self.caculateAngle(center, image.shape[1], line)
        return angle, center
...
class ImageProcessing(TFModels, Control):
    frame_num = 0
    ...
    def __init__(self):
        ...
        # output folder
        self.oDir = self.path+'/output/'+time.strftime("%Y-%m-%d_%H-%M-%S")
        self.oDirs = {
            'rgb': self.oDir+'/rgb',
    ...
    def process_rgb_image(self):
        ...
        angle, center = self.predictCenter(cf.img_rgb[height//3:, :], cf.line)
...
class App(ImageProcessing, Control):
