...
cf.is_barrier_infront = True # = True when something is in front of the sensor. = False when sensor 2 detects barrier open. Used everytime bring the car to the start position to redetect barrier open and restart everything
...
# cf.FIXED_SPEED_route_difficult = 15 # route kho di toc do nay
# cf.SPEED_PASS_OBJ = 21 # toc do chay de vuot vat can
# cf.FIXED_SPEED_TURN = 14 # toc do khi re
# cf.speed_reduced = 15 # toc do thap
# cf.speed_chuyen_lane = 17 # 13 # toc do de chuyen lane
# cf.speed_detect_sign = 16 # toc do di de bat bien bao
# cf.MAX_SPEED = 17
...
cf.do_chuyen_lane = 0
cf.tick_stop_chuyen_lane = 11
cf.goc_chuyen_lane = 40
cf.tick_stop_giu_lane = 7
# cf.max_angle_giu_lane = 30
cf.run_lidar = False
cf.do_detect_obj = False
cf.start_unsee_object = None
...
class Control(Publish):
  ...
  def run_car_by_signal(self):
    ...
    # while cf.ready is False or cf.is_barrier_infront is True:
    while cf.ready is False:
      if cf.warmup is True:
        break
      
      if cf.ready is False:
        self.print_lcd('Wait... | pause', cf.pause, 'is_barrier_infront', cf.is_barrier_infront)
      elif cf.is_barrier_infront is True:
        self.print_lcd("Barrier is in front")
      if cf.ready is True and cf.is_barrier_infront is False:
        break
...
class AutoControl(Control):
  ...
  def speedControl(self, angle, fixed_speed=None):
    ...
    if fixed_speed is not None:
      ...
      if abs(angle) > 18:
        if cf.SAN == 'trai' and len(cf.turned) == 7:
          speed = cf.FIXED_SPEED_TURN - 2
        else:
  ...
  def auto_control__phai(self, angle_from_center):
      ...
      if cf.start_tick_count is not None and not cf.is_barrier_infront:
      ...
      #############################
        ...
        if cf.do_chuyen_lane == 1: # chuyen trai
          goc_chuyen_lane = cf.goc_chuyen_lane
          if cf.lane == 'phai':
            cf.lane = 'trai'
            # # check xem co dang o giua duong khong, giua duong thi giam cf.tick_stop_chuyen_lane xuong
            # cf.tick_stop_chuyen_lane = 14
            
            # check goc, neu goc dang lech ve ben phai thi tang thoi gian chuyen lane (cf.tick_stop_chuyen_lane), neu dang lech ve trai thi giam
            if cf.angle < -11: # lech phai
              goc_chuyen_lane = cf.goc_chuyen_lane + 10 # tang goc chuyen lane
              if cf.angle < -18:
                cf.tick_stop_chuyen_lane += 2
            elif cf.angle > 11: # lech trai
              goc_chuyen_lane = cf.goc_chuyen_lane - 10 # giam goc be khi chuyen lane

          cf.angle = goc_chuyen_lane
      ...
      # elif len(cf.turned) > 6 and cf.do_chuyen_lane == 2 and self.tick_giu_lane is not None: # ket thuc chuyen lane phai o vi tri nhay cam!
        ...
        if self.tick_giu_lane > cf.tick_stop_giu_lane:
          ...
          if cf.do_chuyen_lane == -1: # da chuyen tu phai -> trai, dang o lane trai de vuot vat can
            # tang toc de vuot
        ...
        if cf.current_route == 'thang':
          if cf.tick_to_pass_turn is not None:
            ...
            if cf.tick_to_pass_turn > cf.max_tick_to_pass_turn:
              ...
              # cf.run_lidar = True # comment this because now use yolo to detect moving object
              cf.do_detect_obj = True
  ...
  def whereAmI__trai(self):
    ...
    if 270+cf.imu_early > cf.imu_angle > 270-cf.imu_early and len(cf.turned) == 2: # turn third left
      cf.turned.append('right')
      # cf.fixed_speed = cf.speed_reduced
      cf.fixed_speed = 16
      # cf.reduce_speed = 1
    if 180+cf.imu_early > cf.imu_angle > 180-cf.imu_early and len(cf.turned) == 3: # turn right at finish of segment 1
      cf.turned.append('left')
  ...
  def auto_control__trai(self, angle_from_center):
      ...
      #############################
        ...
        if self.tick_giu_lane > cf.tick_stop_giu_lane:
          ...
          if cf.do_chuyen_lane == -1: # da chuyen lane tu phai -> trai, dang o lane trai de vuot vat can
            # tang toc de vuot
            cf.FIXED_SPEED_STRAIGHT = cf.SPEED_PASS_OBJ
        ...
        if cf.tick_to_pass_turn is not None:
          ...
          if cf.tick_to_pass_turn > cf.max_tick_to_pass_turn:
            ...
            # bat lidar
            if cf.current_route == 'trai':
              # cf.run_lidar = True # dung yolo de detect moving object
              cf.do_detect_obj = True
  ...
  def whereAmI(self):
    ...
    if not cf.is_barrier_infront:
      # print('[whereAmI]   >> current_route', cf.current_route, ' | run_lidar', cf.run_lidar, ' | do_detect_obj', cf.do_detect_obj, ' || do_chuyen_lane', cf.do_chuyen_lane, ' | segment_chuyen_lane', cf.segment_chuyen_lane, ' | cf.pass_object', cf.pass_object, ' | len turned', len(cf.turned), '|| signSignal', cf.signSignal, ' | cf.do_detect_sign', cf.do_detect_sign, ' || cf.do_detect_stop_sign', cf.do_detect_stop_sign, '| stopSignal', cf.stopSignal, '| sign_bbox', cf.sign_bbox, ' | lane', cf.lane, ' | imu_angle', cf.imu_angle, ' | turned', cf.turned, ' | speed', cf.speed, ' | angle', cf.angle, ' | fps_count', cf.fps_count)

      print('[whereAmI]   >> current_route', cf.current_route, ' | do_detect_obj', cf.do_detect_obj, ' || do_chuyen_lane', cf.do_chuyen_lane, ' | segment_chuyen_lane', cf.segment_chuyen_lane, ' | cf.pass_object', cf.pass_object, ' || len turned', len(cf.turned), '|| cf.do_detect_sign', cf.do_detect_sign, ' | signSignal', cf.signSignal, ' || cf.do_detect_stop_sign', cf.do_detect_stop_sign, '| stopSignal', cf.stopSignal, '|| sign_bbox', cf.sign_bbox, ' || lane', cf.lane, ' | speed', cf.speed, ' | cf.angle', round(cf.angle, 2))
      # print('[whereAmI]   >> imu_angle', round(cf.imu_angle, 2))
  ...
  def auto_control(self, angle_from_center):
    ...
    #   self.pause()
    # print('\n **** [auto_control] cf.pause', cf.pause, 'cf.is_barrier_infront', cf.is_barrier_infront)
...
class SignDetector(object):
  ...
  def load_model(self):
    ...
    configPath = sys.path[1]+"/models/yolo/sign_object/yolov3-tiny-sign_object.cfg"
    weightPath = sys.path[1]+"/models/yolo/sign_object/yolov3-tiny-sign_object_final.weights"
    metaPath = sys.path[1]+"/models/yolo/sign_object/sign_object.data"
  ...
  def run_yolo_detect(self, image):
    ''' Detect bien bao VA vat can dong '''
    ...
    if len(detections) > 0:
      for detection in detections:
        ...
        # print('***', cls_id, score, boxes)
        if score > 0.8:
        # if True:
          dets.append((cls_id, boxes))

    return dets

  
  def detectSignObject(self, sign_to_detect='normal'):
    if (cf.current_route is None and cf.do_detect_sign is True) or (cf.current_route is not None and cf.do_detect_stop_sign is True) or cf.do_detect_obj is True:
      cf.count_sign_step += 1

      if cf.count_sign_step % cf.sign_detect_step == 0:
        # img_to_detect = cf.img_rgb_raw[0:120, 0:320]
        # found_obj_and_phai_chuyen_lane = 0

        dets = self.run_yolo_detect(cf.img_rgb_raw)
        ...
        if len(dets) > 0:
          for det in dets:
            ...
            if class_id == 'object':
              see_obj = True
              if cf.do_detect_obj is True:
                
                # if cf.do_chuyen_lane == -1: # da chuyen tu phai -> trai, dang o lane trai (da tranh vat chuyen sang lane trai va chua chuyen lai, thi ke me no, ko gui tin hieu chuyen lane nua)
                #   found_obj_and_phai_chuyen_lane = -1
                # else:
                #   found_obj_and_phai_chuyen_lane = 1

                # if found_obj_and_phai_chuyen_lane == 1: # phat hien vat o phia truoc va can chuyen lane
                
                if cf.do_chuyen_lane == 0 or cf.do_chuyen_lane == -2: # Neu chua chuyen hoac da chuyen tu trai -> phai (tom lai neu dang o lane phai)
                  cf.start_unsee_object = 0
              ...
              if (cf.current_route is None and cf.do_detect_sign is True) or (cf.current_route is not None and cf.do_detect_stop_sign is True):
                if class_id == 'phai' and cf.SAN == 'trai':
                  class_id = "trai"
                elif class_id == 'trai' and cf.SAN == 'phai':
                  class_id = "phai"

                for (x1,y1,x2,y2) in boxes:
                  cf.sign_bbox = (x1,y1,x2,y2)
                  cf.sign.append(class_id)

                self.signRecognize(class_id, sign_to_detect)

        if not see_obj and cf.start_unsee_object is not None: # da tung thay object, va bay gio khong thay nua. Co phai da vuot qua roi ko? cf.start_unsee_object de dem, neu > 10 thi coi nhu da vuot qua
          cf.start_unsee_object += 1

          if cf.start_unsee_object > 6: # da vuot qua, chuyen lai lane phai
            if cf.do_chuyen_lane == -1: # chi chuyen lai lane phai neu dang o lane trai
              cf.pass_object = True
              cf.start_unsee_object = None

  def signRecognize(self, sign_class_id, sign_to_detect='normal'):
    # cf.signSignal = None
    # cf.stopSignal = None

    if cf.sign_bbox is not None:
      ...
      cv2.imwrite(sys.path[1]+"/output/img_detect_sign__{}__{}.jpg".format(cf.count_sign_step, time.time()), img_detect_sign)

      # if self.acceptSign(sign_class_id):
      # if y2-y1>30 and x2-x1>30:
      # if (sign_to_detect == 'normal') or (y2-y1>30 and x2-x1>30):
      if (sign_to_detect == 'normal') or (y2-y1>25 and x2-x1>25):
        if sign_to_detect == 'normal' and sign_class_id != 'dung': # or cf.do_detect_sign = True
          ...
          cf.signSignal =  "{}_certain".format(sign_class_id)
        
        if sign_class_id == 'dung':
          cf.do_detect_stop_sign = False

          cf.stopSignal =  "{}_certain".format(sign_class_id)
        
      else:
        if sign_to_detect == 'normal':
          cf.signSignal = sign_class_id
        elif sign_class_id == 'dung':
          cf.stopSignal = sign_class_id
      
      print('>>>>> cf.signSignal', cf.signSignal, 'sign_to_detect', sign_to_detect)
    ...
    while cf.running: # seperate thread
      if cf.got_rgb_image:
        ...
        if cf.fps_count > 29 and not cf.ready:
          ...
          if not cf.ready:
            print('[getCenter] fps', cf.fps_count, ' | cf.is_barrier_infront', cf.is_barrier_infront)
            # time.sleep(0.01)
  ...
  def getSign(self):
    ...
    while cf.running: # seperate thread
      if cf.got_rgb_image and not cf.pause:
        if cf.do_detect_stop_sign is True and len(cf.turned) == 8:
          sign_to_det = 'stop'
        else:
          sign_to_det = 'normal'

        if cf.do_detect_stop_sign is True or cf.do_detect_sign is True or cf.do_detect_obj is True:
          self.signDetector_obj.detectSignObject(sign_to_detect=sign_to_det)
        else:
          time.sleep(0.1)
...
class App(Camera, ImageProcessing, HandControl, AutoControl, Lidar):
  ...
  def listenner(self):
    ...
    cf.sub_btn1 = rospy.Subscriber(
      '/bt1_status', Bool, self.on_get_btn_1, queue_size=1)
    cf.sub_btn2 = rospy.Subscriber(
      '/bt2_status', Bool, self.on_get_btn_2, queue_size=1)
    cf.sub_btn3 = rospy.Subscriber(
      '/bt3_status', Bool, self.on_get_btn_3, queue_size=1)
    cf.sub_sensor2 = rospy.Subscriber(
      '/ss2_status', Bool, self.on_get_sensor2, queue_size=1)
    cf.sub_getIMUAngle = rospy.Subscriber(
      '/imu', Imu, self.on_get_imu, queue_size=1)
    # cf.sub_lidar = rospy.Subscriber(
    #   '/scan', LaserScan, self.on_get_lidar, queue_size=1)
    
  def warmup(self):
    self.print_lcd('2 chon san | 3 quit || 1 reset and start')
    self.listenner()

  def run(self):
    self.print_lcd('1 start/reset | 2 pause | 3 quit')
    cf.running = True
    print('\n\n ****** run called', cf.running)
  ...
  def on_get_sensor2(self, res):
    # print('\n [on_get_sensor2]', res.data)
    if res.data is True and cf.is_barrier_infront is True: # free in front (barrier open) => run!!
      # self.run_car_by_signal()
      cf.pause = False
      cf.is_barrier_infront = False # run
      # cf.sub_sensor2.unregister()
      # cf.sub_sensor2 = None
    elif res.data is False and not cf.pause: # something in front closely
      # print('************** [on_get_sensor2] PAUSE ------------------')
      cf.is_barrier_infront = True
      # self.pause()
      # time.sleep(1.5)
    time.sleep(0.5)
  ...
  def start_(self):
    self.reset_all()
    self.print_lcd('Reset!')
    cf.running = True

    # if cf.sub_sensor2 is None:
    #   self.print_lcd('Ready to run! sensor2 is deactivated. Press s to run')
    # else:
    #   self.print_lcd('Ready to run! Wait for barrier to open')
    #   cf.is_barrier_infront = True
    self.print_lcd('Ready to run! Wait for barrier to open')
    # cf.is_barrier_infront = True
    # cf.pause = False
  ...
  def on_get_btn_1(self, res):
    ...
    if res.data is True: # click
      print('\n \t ******** [btn_1] cf.warmup', cf.warmup)
      if cf.warmup is False:
        self.start_()
      else:
        cf.warmup = False
        self.print_lcd('Loading model and Opening thread...')
        self.run()
        self.print_lcd('Thread opened! Press 1 again!')
    time.sleep(0.5)
  ...
  def on_get_btn_3(self, res):
    ...
    if res.data is True: # click
      print('\n \t **** [btn_3] cf.warmup', cf.warmup)
      if cf.warmup is False:
        self.quit()
        # self.print_lcd('Warmup mode')
        self.print_lcd('2 chon san | 3 quit || 1 reset and start')
        cf.warmup = True
      else:
        self.kill()
  ...
  def choose_san(self):
      if cf.SAN is None or cf.SAN == 'phai':
        self.choose_santrai()
      else:
        self.choose_sanphai()
  def choose_santrai(self):
      cf.SAN = 'trai'
