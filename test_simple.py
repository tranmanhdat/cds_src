...
cf.start_tick_count = None # If this is set to 0, it will be in running straight mode (with fixed angle=0 and speed=MAX_SPEED) and ignore all other calculation. If set to None, it uses predictCenter to calculate angle and set speed.
...
# init node
rospy.init_node('test_simple', anonymous=True, disable_signals=True)
...
cf.lane_model_path = "/models/lane/model_14.02.h5"
# cf.sign_model_path = "/models/signgray.h5"

cf.line_from_bottom = 40
    ...
    def auto_control(self):
        ...
        if True: # uncomment if call auto_control inside ImageProcessing
            ...
            if cf.current_route == 'phai':
                ...
                if cf.imu_angle > 90 and len(cf.turned) == 5: # turn left
                    cf.turned.append('left')
            ...
            if cf.current_route == 'thang':
                ...
                if cf.imu_angle > 0 and len(cf.turned) == 5: # turn right
                    cf.turned.append('left')



            if not cf.pause and cf.ready:
                if cf.start_tick_count is not None and cf.start_tick_count < 7:
                    cf.start_tick_count += 1
                    cf.angle = 0
                    cf.speed = cf.MAX_SPEED
                    print('cf.start_tick_count', cf.start_tick_count)
                else: 
                    cf.start_tick_count = None

            '''
            if cf.signSignal == 'left':
                if cf.signstep == 0:
    ...
    def getCenter(self):
        ...
        # if cf.running: # inside get_rgb
            if cf.got_rgb_image:
                ...
                img_rgb_resized = img_rgb_resized[80:240, :]
                angle, cf.center = self.lane_tfmodel.predictCenter(img_rgb_resized)
                ...
                pt1_end = (w//2, 160)
                cv2.line(img_rgb_resized, pt1_start, pt1_end, red, 2)

                # line predict
                pt2_start = (cf.center, 160-cf.line_from_bottom)
                pt2_end = pt1_end
                cv2.line(img_rgb_resized, pt2_start, pt2_end, green, 2)

                cv2.circle(img_rgb_resized, (cf.center, 160-cf.line_from_bottom), 5, green, -1)
                cv2.putText(img_rgb_resized, 'center: '+str(cf.center)+' - '+str(cf.first_imu_angle), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, red, 2)

                cv2.imshow('img_rgb_resized', img_rgb_resized)
                cv2.imwrite(self.oDirs['depth']+'/{}__{}_{}_{}.jpg'.format(self.frame_num, cf.speed, round(cf.angle,1), round(cf.imu_angle, 1)), img_rgb_resized)
    ...
    def putTextSignal(self):
        if cf.signSignal is not None:
            ...
            if color is not None:
                cv2.circle(cf.rgb_viz, (10*index, 20), size, color, -1)
