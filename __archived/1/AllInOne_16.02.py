...
cf.lane_model_path = "/models/lane/model_5_Center.h5"
    ...
    def auto_control(self):
        ...
        if True: # uncomment if call auto_control inside ImageProcessing
            ...
            if not cf.pause and cf.ready:
                if cf.start_tick_count is not None and cf.start_tick_count < 10:
    ...
    def getCenter(self):
        ...
        # if cf.running: # inside get_rgb
            if cf.got_rgb_image:
                ...
                cf.img_rgb_resized = cf.img_rgb_resized[80:240, :]
    ...
    def getSign(self):
        while cf.running:
            if cf.got_rgb_image:
                ...
                cf.signSignal, cf.sign_bbbox = self.signRecognize()
                print('cf.signSignal', cf.signSignal)
                cf.ready = True
    ...
    def visualize(self):
        ...
        if cf.running: # inside getCenter (run in same thread)
            if cf.got_rgb_image:
                ...
                pt1_end = (w//2, 160)
                cv2.line(cf.img_rgb_resized, pt1_start, pt1_end, red, 2)

                # line predict
                pt2_start = (cf.center, 160-cf.line_from_bottom)
                pt2_end = pt1_end
                cv2.line(cf.img_rgb_resized, pt2_start, pt2_end, green, 2)

                cv2.circle(cf.img_rgb_resized, (cf.center, 160-cf.line_from_bottom), 5, green, -1)
                cv2.putText(cf.img_rgb_resized, 'predict: '+str(cf.predict), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, red, 2)
                cv2.putText(cf.img_rgb_resized, 'center: '+str(cf.center)+' - '+str(cf.first_imu_angle), (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, red, 2)

                cv2.imshow('cf.img_rgb_resized', cf.img_rgb_resized)
                cv2.imwrite(self.oDirs['depth']+'/{}__{}_{}_{}.jpg'.format(self.frame_num, cf.speed, round(cf.angle,1), round(cf.imu_angle, 1)), cf.img_rgb_resized)
