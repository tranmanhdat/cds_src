...
trt._deprecated_helpers.SUPPRESS_DEPRECATION_WARNINGS=True
...
cf.sign_model_path = "/models/signs/sign_5.engine"
...
cf.lane_model_path = "/models/lane_draw/test_tf.engine"
...
class TrtNet(object):
    ...
    """
    def __init__(self, engine_path, input_size, output_size):
        self.engine_path = engine_path
        self.output = np.empty(output_size, dtype=np.float32)
    
        self.G_LOGGER = trt.infer.ConsoleLogger(trt.infer.LogSeverity.ERROR)
        self.engine = trt.utils.load_engine(self.G_LOGGER, sys.path[1]+self.engine_path)
    ...
    def trt_predict(self, img):
        ...
        # print(img.shape, img.size, img.dtype.itemsize)
    ...
    def signRecognize(self, sign_to_detect='normal'):
        ...
        if run:
            ...
            for (x1,y1,x2,y2) in boxes:
                ...
                output = self.sign_net.trt_predict(np.array([img])/255.0)[0]
                print('\t output', output)

                sign_class_id = np.argmax(output)
                prob = output[sign_class_id]
                # print(output, sign_class_id, prob)
                if prob > 0.97:
...
class CenterDetector(object):
    ...
    def findCenterAdvance(self):
        ...
        output = self.lane_net.trt_predict(np.array([img])/255.0)
        cf.predict = output[0]
...
class ImageProcessing(AutoControl):
    ...
    def getCenter(self):
        self.centerDetector_obj.lane_net = TrtNet(cf.lane_model_path, input_size=(180,320,3), output_size=(1,1))
    ...
    def getSign(self):
        self.signDetector_obj.sign_net = TrtNet(cf.sign_model_path, input_size=(100,100,3), output_size=(1,3))
            ...
            if cf.got_rgb_image:
                ...
                if cf.do_detect_sign:
                    if cf.signSignal is None or 'certain' not in cf.signSignal:
                        cf.signSignal, cf.sign_bbbox = self.signDetector_obj.signRecognize()
                    ...
                    cf.sign_bbbox = None
                    self.k = 0
                    cf.stopSignal, cf.sign_bbbox = self.signDetector_obj.signRecognize('stop')
