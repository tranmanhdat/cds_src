...
# cf.lane_model_path = "/models/lane/draw_320x200__160+40.h5"
cf.lane_model_path = "/models/lane_draw/draw_320x180__140+40__new_model_1.h5"
...
cf.Q_center = Queue(maxsize=1)
cf.Q_turn = Queue(maxsize=1)
cf.Q_sign = Queue(maxsize=1)
...
class TurnDetector(object):
    ...
    def detectTurn(self):
        ...
        img_rgb_raw = cf.Q_turn.get()
        cropped_raw_image = self.region_of_interest(img_rgb_raw, v)
...
class SignDetector(object):
    ...
    def signRecognize(self, sign_to_detect='normal'):
        ...
        if run:
            img_rgb_raw = cf.Q_sign.get()
            img_detect_sign = img_rgb_raw[cf.detect_sign_region['top']:cf.detect_sign_region['bottom'], cf.detect_sign_region['left']:cf.detect_sign_region['right']]
...
class ImageProcessing(SignDetector, TurnDetector, AutoControl):
    ...
    def getCenter(self):
        ...
        # if cf.running: # inside get_rgb
            # if cf.got_rgb_image:
            if cf.Q_center.qsize() > 0:
                cf.img_rgb_raw = cf.Q_center.get()
                ...
                cf.center = int(predict[0]*cf.img_rgb_resized.shape[1])
                angle = self.caculateAngle(cf.center, cf.img_rgb_resized.shape[1])
        ...
        while cf.running: # seperate thread
            if cf.got_rgb_image:
                ...
                if cf.do_detect_sign:
                    ...
                    cf.sign_bbbox = None
                    self.k = 0
                    cf.stopSignal = self.stopRecognize()
    ...
    def draw(self):
        ...
        # draw fps
        # cv2.putText(cf.rgb_viz, 'Elasped time: '+str(cf.fps.elapsed()), (300, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, blue, 2)
        # cv2.putText(cf.rgb_viz, 'FPS: '+str(cf.fps.fps()), (300, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, blue, 2)
...
class Camera(ImageProcessing):
    ...
    def get_rgb(self):
        ...
        while cf.running:
            ...
            img_rgb_raw = rgb[:, ::-1, :]  # flip

            if cf.Q_center.full():
                cf.Q_center.get()
            cf.Q_center.put(img_rgb_raw)
            if cf.Q_turn.full():
                cf.Q_turn.get()
            cf.Q_turn.put(img_rgb_raw)
            if cf.Q_sign.full():
                cf.Q_sign.get()
            cf.Q_sign.put(img_rgb_raw)
