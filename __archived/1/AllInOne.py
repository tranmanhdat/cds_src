...
cf.sign_model_path = "/models/signs/sign_tf_FP16"
...
# cf.lane_model_path = "/models/lane_draw/draw_320x200__160+40.engine"
cf.lane_model_path = "/models/lane_draw/test_tf_FP16"
...
class AutoControl(Control):
    ...
    def whereAmI(self):
        if cf.signSignal is not None: # sign detected! change the current route!
            cf.current_route = cf.signSignal.split('_')[0]

        if cf.imu_angle > 90-cf.imu_early and len(cf.turned) == 0: # turn first left
            cf.turned.append('left')
        if cf.imu_angle > 180-cf.imu_early and len(cf.turned) == 1: # turn second left
            cf.turned.append('left')
        if cf.imu_angle > 270-cf.imu_early and len(cf.turned) == 2: # turn third left
            cf.turned.append('left')
            cf.reduce_speed = True
        if cf.imu_angle > 180-cf.imu_early and len(cf.turned) == 3: # turn right at finish of segment 1
        ...
        if cf.current_route == 'phai':
            ...
            if cf.imu_angle > 90-cf.imu_early and len(cf.turned) == 4: # turn right
                cf.turned.append('right')
                cf.reduce_speed = False

            if cf.imu_angle > 180-cf.imu_early and len(cf.turned) == 5: # turn left
                cf.turned.append('left')
                cf.current_route = 'phai'
            if cf.imu_angle > 270-cf.imu_early and len(cf.turned) == 6: # turn left
                cf.turned.append('left')
            if cf.imu_angle > 360-cf.imu_early and len(cf.turned) == 7: # turn left
                cf.turned.append('left')
        ...
        print('   || ', cf.current_route, cf.imu_angle, cf.turned)
...
class SignDetector(object):
    ...
    # sign_net = TrtNet(cf.lane_model_path, input_size=(100,100,3), output_size=(1,2))
...
class CenterDetector(object):
    ...
    def __init__(self):
        ...
        # self.createNet(cf.lane_model_path, input_size=(180,320,3), output_size=(1,1))
    ...
    def findCenterAdvance(self):
        ...
        cf.predict = output[0]
        center = int(output[0]*cf.img_rgb_resized.shape[1])
...
class ImageProcessing(AutoControl):
    ...
    def __init__(self):
        ...
        self.turnDetector_obj = TurnDetector()
    ...
    def getCenter(self):
        ...
        # if cf.running: # inside get_rgb
            ...
            if cf.got_rgb_image:
                cf.center, angle = self.centerDetector_obj.findCenterAdvance()
    ...
    def getTurn(self):
        while cf.running: # seperate thread
            if cf.got_rgb_image:
                cf.turnSignal = self.turnDetector_obj.detectTurn()
