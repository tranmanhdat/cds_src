...
cf.FIXED_SPEED_TURN = 16 # toc do khi re
cf.end_tick_from_start = 37 # tick de di thang doan dau #20(for 17,14) # 22(for 14,12)
cf.MAX_SPEED = 25
# cf.MAX_SPEED = 16
cf.MIN_SPEED = 14
...
cf.speed_detect_sign = 15 # toc do di de bat bien bao
...
class AutoControl(Control):
    ...
    def auto_control__phai(self, angle_from_center):
            ...
            if cf.start_tick_count is not None:
                if cf.start_tick_count < cf.end_tick_from_start:
                    ...
                    cf.angle = 0
                    if cf.start_tick_count % 3 == 1 and cf.fixed_speed < cf.MAX_SPEED:
                        cf.fixed_speed += 1
                else:
    ...
    def auto_control__trai(self, angle_from_center):
            ...
            #############################
                ...
                if len(cf.turned) == 8 and self.tick_giu_lane is None and cf.do_chuyen_lane == -2 and angle_from_center < -20:
    ...
    def whereAmI(self):
        ...
        print('   >> current_route', cf.current_route, ' | signSignal', cf.signSignal, ' || cf.do_detect_stop_sign', cf.do_detect_stop_sign, '| stopSignal', cf.stopSignal, '| sign_bbox', cf.sign_bbox, ' | lane', cf.lane, ' | imu_angle', cf.imu_angle, ' | turned', cf.turned, ' | speed', cf.speed, ' | angle', cf.angle, ' | fps_count', cf.fps_count)
...
class SignDetector(object):
    ...
    def signDetector(self, img, color="blue"):
        ...
        if color == 'red':
            ...
            lower = np.array([0, 68, 0]) 
            upper = np.array([25,255,255]) # 100, 97, 45
        ...
        for contour in contours:
            ...
            if 100 > w >= 15 and 100 > h >= 15 and abs(w-h) < 5 and abs(h-w) < 5:   
            ...
            if cf.count_sign_step % cf.sign_detect_step == 0:
                ...
                for (x1,y1,x2,y2) in boxes:
                    ...
                    if prob > 0.7:
                        ...
                        # cv2.imwrite(sys.path[1]+"/output/sign_region__{}.jpg".format(cf.count_sign_step), sign_region)
    ...
    def stopSignRecognize(self):
        if cf.do_detect_stop_sign:
            cf.count_sign_step += 1
            ...
            if cf.count_sign_step % cf.sign_detect_step == 0:
                ...
                for (x1,y1,x2,y2) in boxes:
                    sign_region = img_detect_sign[y1:y2, x1:x2]
                    cropped = cv2.resize(sign_region, (40, 40))

                    cropped_norm = preprocessing.normalize(cropped.reshape(1,-1))
                    template_norm = preprocessing.normalize(self.template.reshape(1,-1))
                    ...
                    if s > 0.8:
                        ...
                        cv2.imwrite(sys.path[1]+"/output/stop__{}.jpg".format(cf.count_sign_step), sign_region)

                        # if self.acceptSign(2):
                        #     return "dung_certain"
                        
                        return "dung_certain", (x1,y1,x2,y2)
        return None, None
    ...
    def getSign(self):
        ...
        while cf.running: # seperate thread
            if cf.got_rgb_image and not cf.pause:
                # print('getSign | cf.do_detect_sign =', cf.do_detect_sign, ' | cf.turned ', len(cf.turned))
                if cf.do_detect_sign is True:
                ...
                elif cf.do_detect_stop_sign:
                    # if len(cf.turned) == 8:
                    cf.stopSignal, cf.sign_bbox = self.signDetector_obj.stopSignRecognize()
    ...
    def get_depth(self):
        while cf.running:
            if not cf.ready or len(cf.turned) < 7:
                ...
                print('[get_depth]', cf.stopSignal, cf.sign_bbox)
                ...
                if cf.stopSignal is not None and cf.sign_bbox is not None:
                    x1,y1,x2,y2 = cf.sign_bbox
                    x1 = x1+5+cf.detect_sign_region['left']
