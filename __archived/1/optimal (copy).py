...
# Model
cf.sign_model_path = "/models/signs/sign_santrai_tf__stop_trt_FP16"
...
cf.FIXED_SPEED_route_difficult = 17 # route kho di toc do nay
cf.SPEED_PASS_OBJ = 20 # toc do chay de vuot vat can
cf.FIXED_SPEED_TURN = 15 # toc do khi re
cf.end_tick_from_start = 30 # tick de di thang doan dau #20(for 17,14) # 22(for 14,12)
cf.MAX_SPEED = 25
# cf.MAX_SPEED = 16
cf.MIN_SPEED = 14
...
class AutoControl(Control):
    ...
    def auto_control__trai(self, angle_from_center):
            ...
            #############################
                ...
                if len(cf.turned) == 8 and self.tick_giu_lane is None and cf.do_chuyen_lane == -2 and angle_from_center < -25:
    ...
    def whereAmI(self):
        ...
        print('   >> current_route', cf.current_route, ' | signSignal', cf.signSignal, ' | lane', cf.lane, ' | imu_angle', cf.imu_angle, ' | turned', cf.turned, ' | speed', cf.speed, ' | angle', cf.angle, ' | fps_count', cf.fps_count)
...
class SignDetector(object):
    ...
    def signDetector(self, img, color="blue"):
        ...
        for contour in contours:
            ...
            if 100 > w >= 20 and 100 > h >= 20 and abs(w-h) < 5 and abs(h-w) < 5:   
                ...
                for (x1,y1,x2,y2) in boxes:
                    ...
                    if prob > 0.75:
                        ...
                        if self.acceptSign(sign_class_id):
                            ...
                            cv2.imshow('sign area', img_detect_sign[y1:y2, x1:x2])
        ...
        while cf.running:
            if cf.turn_on_depth:
                ...
                if cf.signSignal is not None:
                    ...
                    sign_depth = img_depth[y1+cf.detect_sign_region['top']:y2+10+cf.detect_sign_region['top'], x1+5+cf.detect_sign_region['left']:x2+20+cf.detect_sign_region['left']]

                    cv2.imshow('sign_depth', sign_depth)
                    sd = sign_depth.ravel()
                    x = sd[(cf.low_thresh < sd) & (sd < cf.up_thresh)]
                    print('x', x)
                    print('sd.shape', sd.shape, 'x.shape', x.shape)
                    if len(x) > len(sd)*2/3:
