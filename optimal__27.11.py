...
class AutoControl(Control):
  ...
  def whereAmI(self):
    ...
    if not cf.is_barrier_infront:
      ...
      # print('[whereAmI]   >> current_route', cf.current_route, ' | do_detect_obj', cf.do_detect_obj, ' || do_chuyen_lane', cf.do_chuyen_lane, ' | segment_chuyen_lane', cf.segment_chuyen_lane, ' | cf.pass_object', cf.pass_object, ' || len turned', len(cf.turned), '|| cf.do_detect_sign', cf.do_detect_sign, ' | signSignal', cf.signSignal, ' || cf.do_detect_stop_sign', cf.do_detect_stop_sign, '| stopSignal', cf.stopSignal, '|| sign_bbox', cf.sign_bbox, ' || lane', cf.lane, ' | speed', cf.speed, ' | imu_angle', round(cf.imu_angle, 2))
...
class SignDetector(object):
  ...
  def signRecognize(self, sign_class_id, sign_to_detect='normal'):
    ...
    if cf.sign_bbox is not None:
      ...
      if (sign_to_detect == 'normal') or (y2-y1>20 and x2-x1>20):
