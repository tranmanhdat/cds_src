...
class AutoControl(Control):
    ...
    def whereAmI(self):
        ...
        print('[whereAmI]   >> current_route', cf.current_route, ' | signSignal', cf.signSignal, ' | cf.do_detect_sign', cf.do_detect_sign, ' || cf.do_detect_stop_sign', cf.do_detect_stop_sign, '| stopSignal', cf.stopSignal, '| sign_bbox', cf.sign_bbox, ' | lane', cf.lane, ' | imu_angle', cf.imu_angle, ' | turned', cf.turned, ' | speed', cf.speed, ' | angle', cf.angle, ' | fps_count', cf.fps_count)
...
class SignDetector(object):
    ...
    def signRecognize(self, sign_to_detect='normal'):  
        if cf.detect_sign_region is not None:
            ...
            for (x1,y1,x2,y2) in boxes:
                ...
                cv2.rectangle(img_detect_sign,(x1,y1),(x2,y2),)
                cv2.imshow("sign",img_detect_sign)
                cv2.waitKey(1)
...
class ImageProcessing(AutoControl):
    ...
    def getSign(self):
        ...
        print('[getSign processing]')
        time.sleep(0.005)
        while True: # seperate thread
            if cf.got_rgb_image:
